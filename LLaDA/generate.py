...
def generate(model, prompt, steps=128, gen_length=128, block_length=128, temperature=0.,
    ...
    for num_block in range(num_blocks):
        ...
        for i in range(steps):
            ...
            # reasoning loop
            for reasoning_step in range(0):
                combined_embeds = torch.cat([
                    prompt_embeds,  # Keep original prompt embeddings
                    hidden_states[-1][:, prompt.shape[1]:]  # Use refined hidden states for generation
                ], dim=1)
        
                out = model(inputs_embeds=combined_embeds, output_hidden_states=True)
                hidden_states = out.hidden_states
